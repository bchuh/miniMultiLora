...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        embeddings = embeddings.sum(1) / SENTENCE_LENGTH
        embeddings = embeddings.view(BATCH_SIZE, embeddings.shape[-1])
        output = self.layer1(embeddings)
        output = output.relu()
        minitorch.dropout(output, self.dropout_prob)
...
def BCELoss(y_true, y_pred):
    # one = minitorch.tensor(np.ones(y_pred.shape), backend=BACKEND, requires_grad=True)
    one = minitorch.tensor(np.ones(y_pred.shape).tolist(), backend=BACKEND, requires_grad=True)
    one_y_pred = (one-y_pred)
    out = y_true*y_pred.log() + (minitorch.tensor(np.ones(y_true.shape).tolist(), backend=BACKEND, requires_grad=True)-y_true)*one_y_pred.log()
    return -out.mean(0)
    ...
    def train(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for batch_num, example_num in enumerate(
                ...
                # BEGIN ASSIGN1_4
                # TODO
                batch_end_idx = min(batch_num + batch_size, n_training_samples)
                x = X_train[example_num:batch_end_idx]
                y = y_train[example_num:batch_end_idx]
                if (len(x)) ==0:
                  continue
                x = minitorch.tensor(x, backend=BACKEND, requires_grad=True)
                y = minitorch.tensor(y, backend=BACKEND, requires_grad=True)
                out = model(x)
                loss = BCELoss(y, out)
                assert loss.size == 1, print(loss.size)
                loss.backward()
                optim.step()
            ...
            if data_val is not None:
                ...
                out_val = model(x)
                pred_arr = get_predictions_array(y, out_val)
                validation_predictions += pred_arr
                validation_accuracy.append( get_accuracy(pred_arr))
                # END ASSIGN1_4
