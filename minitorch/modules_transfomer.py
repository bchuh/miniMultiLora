...
class MultiHeadAttention(Module):
    ...
    def self_attention(self, q, kT, v):
        ...
        ### BEGIN YOUR SOLUTION
        q_kt = q @ kT
        inside_softmax = q_kt / math.sqrt(self.attn_hidden_dim)
        if self.causal:
            mask = self.create_causal_mask(queries_len)
            inside_softmax += mask
        result = softmax(inside_softmax, 3) @ v
    ...
    def forward(self, x):
        ...
        after_att = self.self_attention(q, kT, v)
        result = self.dropout(self.out_projection(after_att))
        return result
        ### END YOUR SOLUTION
...
class DecoderLM(Module):
    ...
    def forward(self, idx):
        ...
        """
        position_ids = np.arange(0, seq_len)[np.newaxis, :]
        position_ids = tensor_from_numpy(position_ids, backend=self.backend, requires_grad=False)
        pos_embd = self.position_embeddings(position_ids)
        assert pos_embd.shape == (1, seq_len, self.n_embd)
        embd = self.token_embeddings(idx) + pos_embd
        embd = self.dropout(embd)
        x = self.t_layer_1(embd)
        x = self.t_layer_2(x)
        x = self.t_layer_3(x)
        x = self.t_layer_4(x)
