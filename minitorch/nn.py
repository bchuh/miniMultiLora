...
def GELU(input: Tensor) -> Tensor: 
    ...
    """
    ### BEGIN YOUR SOLUTION
    x_tanh = math.sqrt(2. / math.pi) * (input + 0.044715 * input**3)
    return 0.5 * input * (1 + x_tanh.tanh())
    ### END YOUR SOLUTION
...
def one_hot(input: Tensor, num_classes: int) -> Tensor:
    ...
    one_hot_encoded = identity[input.to_numpy().astype(int)]
    return tensor_from_numpy(one_hot_encoded, input.backend)
    ### END YOUR SOLUTION
...
def softmax_loss(logits: Tensor, target: Tensor) -> Tensor:
    ...
    target_one_hot = one_hot(target, num_classes=logits.shape[-1])
    mult = target_one_hot * logits
    result = logsumexp(logits, 1) - mult.sum(1)
    ### END YOUR SOLUTION
